var domManipulation = function(){
    function appendChild(child, parent) {
        ...
        if (this.parent !== null) {
            for (var i = 0, len = this.parent.length; i < len; i ++) {
    ...
    function addHandler(element, childEventType, eventHandler) {
        this.parent = retrieveElements(element);
        if (this.parent !== null) {
            for (var i = 0, len = this.parent.length; i < len; i += 1) {
                this.parent[i].addEventListener(childEventType, eventHandler);
            }
        }
    }
...
var elements = domManipulation.retrieveElements(".bird");
